...
def test_build_engine_wires_config_into_engine():
    config = _config(grace_period_seconds=42, tier1_hours=1, tier2_days=2, prescription_days=3)
    db = Database(":memory:")
    gw = FakeGateway()
...
def test_start_scheduler_registers_expected_jobs():
    ...
    scheduler = start_scheduler(db, gw, config)
    try:
        job_ids = {job.id for job in scheduler.get_jobs()}
        assert job_ids == {"process_returns", "weekly_recap"}
    finally:
